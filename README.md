...
> **What's new in v0.3.7** — Fixed `raff template list --category` flag help: real values are `os` (Linux/Windows) and `marketplace` (pre-baked apps), not `linux/windows`. Earlier 0.3.x patches also added `MAC` column on `raff vm networks`, `raff vm create --no-vpc`, base-disk vs attached-volumes split on `raff vm get`, "Attached VMs" relabel on security groups, plus the API-side auto-derive that lets `raff volume create` work without `--pricing-id` and honors `--filesystem ext4`. Full details in the [API changelog](https://docs.rafftechnologies.com/api-reference/changelog).
...
Replace `0.3.7` below with the version you want from the [Releases page](https://github.com/RaffTechnologies/raff-cli/releases).

```bash
# Linux x86_64
curl -sL https://github.com/RaffTechnologies/raff-cli/releases/download/v0.3.7/raff_0.3.7_linux_x86_64.tar.gz | tar -xzv
sudo mv raff /usr/local/bin/
```

```bash
# macOS arm64 (Apple Silicon)
curl -sL https://github.com/RaffTechnologies/raff-cli/releases/download/v0.3.7/raff_0.3.7_macos_arm64.tar.gz | tar -xzv
sudo mv raff /usr/local/bin/
```
